...
V2 approval requests: 14, bundled ratio: 1
...
The report is intentionally evidence-led: reproduce it with `node scripts/evaluate-prompt-governance.mjs --json`. The V1 figures are a deterministic compatibility baseline over the same fixtures, not a claim about an unobserved live conversation.
